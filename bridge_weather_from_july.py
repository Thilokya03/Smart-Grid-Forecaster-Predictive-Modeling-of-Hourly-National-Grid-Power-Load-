...
BRIDGE_OUTPUT_FILE = Path("july_bridge_weather_data.csv")
LAST_RUN_FILE = Path("july_bridge_last_run.txt")
...
def get_next_start_time(end_time):
    if BRIDGE_OUTPUT_FILE.exists():
        ...
        complete_hours = counts[counts == len(CITIES)].index
...
def validate_bridge_data(new_data, start_time, end_time):
    ...
    incomplete_hours = counts[counts < len(CITIES)]

    if not incomplete_hours.empty:
        print("Warning: some hours do not have all 25 districts.")
...
def main():
    ...
    for city, (latitude, longitude) in tqdm(CITIES.items()):
    ...
    bridge_data = pd.concat(all_city_data, ignore_index=True)
