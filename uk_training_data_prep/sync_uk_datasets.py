...
def save_metadata(release, checksums):
    ...
    metadata = {
        ...
        "filtered_start_date": FILTER_START_DATE,
        "datasets": {
            filename: {
                "sha256": checksums.get(filename),
                "filtered_copy": FILTERED_DATASET_NAMES.get(filename),
            }
...
# ============================================================
# Save filtered local copies
# ============================================================

def save_filtered_dataset(source_path):
    """
    Save a project-ready copy beginning at FILTER_START_DATE.

    The original release asset is kept unchanged, and the filtered
    file is used by the master training dataset builder.
    """

    filtered_name = FILTERED_DATASET_NAMES.get(source_path.name)
    ...
    if "date" not in frame.columns:
        raise ValueError(
            f"{source_path.name} must contain a date column "
            "to create the filtered copy."
        )
    ...
    filtered = frame[
        frame["date"] >= pd.Timestamp(FILTER_START_DATE)
    ].copy()
    ...
    destination = DOWNLOAD_DIR / filtered_name
    ...
    print(
        f"Saved filtered copy from {FILTER_START_DATE}: "
        f"{destination.name}"
    )
    ...
    for dataset_name in DATASET_NAMES:
        ...
        if destination.exists():
            ...
            if (
                ...
                save_filtered_dataset(destination)
