...
PREDICTIONS_PATH = (
    OUTPUT_DIR / "dnn_predictions.csv"
)

ALL_HORIZONS_PATH = (
    OUTPUT_DIR / "dnn_predictions_all_horizons.csv"
)

METRICS_PATH = (
    OUTPUT_DIR / "dnn_metrics.json"
)

FOLD_METRICS_PATH = (
    OUTPUT_DIR / "dnn_validation_metrics.csv"
)
...
#
# IMPORTANT:
# June 2026 is NOT included in any fold.
# ============================================================
...
def set_seed():
    ...
    # Deterministic mode gives reproducible results.
...
def create_fold_windows(
    ...
    for start in range(
        ...
        # TRAINING
        #
        # Entire 24-hour target horizon must be before
        ...
        # VALIDATION
        #
        # Entire 24-hour horizon must be inside the fold.
        # ----------------------------------------------------
...
def main():
    ...
    for (
        ...
        if validation_end >= FINAL_TEST_START:

            raise RuntimeError(
                f"{fold_name} overlaps June "
                "or later data."
            )
        ...
        for epoch in range(
            ...
            # Keep the best epoch within this fold.
        ...
        print(
            f"MAE  : "
            f"{fold_metric['mae']:.4f}"
        )

        print(
            f"RMSE : "
            f"{fold_metric['rmse']:.4f}"
        )

        print(
            f"MAPE : "
            f"{fold_metric['mape']:.4f}%"
        )
